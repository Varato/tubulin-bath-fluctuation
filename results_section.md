...
**Figure 3.** Population $P_7(t)$ starting from Trp4. (a) Noise-component ablation. (b) Trajectory MC with full MD noise (black) versus the Haken–Strobl Lindblad at $\gamma=50$ cm⁻¹ (dashed blue); the green curve drives the MC in the white-noise limit ($\sigma^2\tau=\gamma$) and converges onto the Lindblad result, validating the code.
...
**Comparison to the Markovian treatment.** Craddock (2014) modelled dephasing with a Haken–Strobl Lindblad at a fixed rate $\gamma=50$ cm⁻¹ ($1/\gamma=106$ fs). On the same dimer this predicts $P_7(2\,{\rm ps})=0.50$, markedly higher than the exact coloured-noise result (0.31). As a validation, we drive the trajectory MC with a single symmetric Ornstein–Uhlenbeck process on each site, choosing $\tau=5$ fs and $\sigma\approx 230$ cm⁻¹ so that the white-noise dephasing rate $\sigma^2\tau$ equals the Lindblad rate $\gamma=50$ cm⁻¹. This equality holds in natural $\hbar{=}1$ units, where wavenumbers convert to angular frequency via $2\pi c$: $\gamma=9.4$ ps⁻¹ and $\sigma=43$ ps⁻¹, giving $\sigma^2\tau=43^2\times 0.005=9.2$ ps⁻¹$\,\approx\gamma$ (and $\tau=5$ fs $\ll 1/J\approx 90$ fs, deep in the white-noise limit). The MC then converges onto the Lindblad curve ($P_7(2\,{\rm ps})=0.50\pm0.01$; solid blue, Fig. 3b), confirming both the code and that the MD–Lindblad discrepancy is physical rather than numerical. The discrepancy is the non-Markovian regime: the Kubo numbers $\kappa_k=\sigma_k T_k/\hbar$ are $\gg 1$ for all three components ($\sim$6 for $T_1$, $\sim 10^2$ for $T_2$, $\sim 10^5$ for $T_3$), so the white-noise assumption underlying the Lindblad master equation is violated. A $\sigma$-scan makes this quantitative: reproducing the Lindblad transfer $P_7=0.50$ with the realistic tri-exponential noise requires scaling the disorder to $\alpha\approx 0.12$ of its MD value ($\sigma_{\rm eff}\approx 120$ cm⁻¹ $\approx 2J$), i.e. $\gamma=50$ sits near the environment-assisted-transport optimum, whereas the true MD bath ($\sigma\approx 1000$ cm⁻¹ $\approx 17J$) lies deep in the overdamped regime. The Markovian rate is thus a fit parameter, not an ab initio prediction of the bath.
